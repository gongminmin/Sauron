...
{
	// TODO
	return false && EphemWrapper::jd_fits_de430(jd);
}

bool use_de431(const double jd)
{
	// TODO
	return false && EphemWrapper::jd_fits_de431(jd);
}
...
void get_planet_helio_coordsv(const double jd, double xyz[3], double xyzdot[3], const int planet_id)
{
	...
	if(!std::isfinite(jd))
	{
		std::cerr << "get_planet_helio_coordsv(): SKIPPED CoordCalc, jd is infinite/nan: " << jd << std::endl;
		return;
	}
...
void get_planet_helio_osculating_coordsv(double jd0, double jd, double xyz[3], double xyzdot[3], int planet_id)
{
	...
	{
		std::cerr << "get_planet_helio_osculating_coordsv(): SKIPPED CoordCalc, jd0 or jd is infinite/nan. jd0:" << jd0 << "jd: "<< jd << std::endl;
		return;
	}
...
{
	SAURON_UNUSED(unused);
	bool deOk=false;
	double xyz6[6];
	if(!std::isfinite(jd))
	{
		std::cerr << "get_pluto_helio_coordsv(): SKIPPED PlutoCoordCalc, jd is infinite/nan:" << jd << std::endl;
		return;
	}
...
void get_earth_helio_coordsv(const double jd,double xyz[3], double xyzdot[3], void* unused)
{
	...
	if(!std::isfinite(jd))
	{
		std::cerr << "get_earth_helio_coordsv(): SKIPPED EarthCoordCalc, jd is infinite/nan:" << jd << std::endl;
		return;
	}
