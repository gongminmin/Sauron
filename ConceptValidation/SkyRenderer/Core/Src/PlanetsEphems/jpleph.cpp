                          ...
                          char nam[][6], double *val)
{
    ...
    {
      std::cout << "[DEBUG] Byte order is wrong for current platform" << std::endl;
    ...
    {
          std::cout << "[WARNING] " << "jpl_init_ephemeris(): Cannot seek in file. Result will be undefined." << std::endl;
    }
      }
    ...
    {   
      ...
      std::cout << "[WARNING] " << "temp_data: " << temp_data.emrat << "(should have been =~81). JPL_INIT_FILE_CORRUPT!" << std::endl;
    }
...
double DLL_FUNC jpl_get_constant(const int idx, void *ephem, char *constant_name)
{
    ...
    *constant_name = '\0';
    if(idx >= 0 && idx < (int)eph->ncon)
    {
        ...
        {
            std::cout << "[WARNING]  jpl_get_constant(): Cannot seek in file. Result will be undefined." << std::endl;
        }
        if(fread(constant_name, 1, 6, eph->ifile) == 6 )
        {
            ...
            {
                std::cout << "[WARNING] jpl_get_constant(): Cannot seek in file (call2). Result will be undefined." << std::endl;
            }
            ...
            if( fread(&rval, 1, sizeof(double), eph->ifile) == sizeof(double) )
            {
                ...
                std::cout << "[WARNING] jpl_get_constant(): fread() failed to read a double. Result will be undefined." << std::endl;
        }
        else
            std::cout << "[WARNING] jpl_get_constant(): fread() failed to read a constant name. Result will be 0, likely wrong." << std::endl;
    }
    return(rval);
}
/*************************** THE END ***************************************/
